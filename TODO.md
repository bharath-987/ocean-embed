...
- [x] **Task: Second Half of Ajay's Pre-Review List — New Features & Final-Days Polish (Items 9–16)** `[Completed 2026-09-20 23:31]`
  - [x] **Item 9: Fingerprint Atlas (`GET /regimes?date=`)**:
    ...
    - Created `fingerprint.html` and `fingerprint.js` featuring interactive 8-cluster regime segmentation, MapLibre GL raster tile rendering, SLA brightness modulation toggle, interactive chips, and Chart.js relative profile graph.
    - Explicit disclosure prominently integrated in header banner and legend: `"colours are relative, not physical units"`. Scope boundary respected: NO "find similar days" search feature built.
  - [x] **Item 10: Storm Heat Replay (`GET /products-grid?name=...&date=`)**:
    - Built `GET /products-grid?name=tchp|d26|d20|mld&date=` returning memory-mapped product fields via vectorized numpy array serialization (`np.where(np.isfinite(...))`).
    - Added TCHP day slider and 5 storm date jump buttons in `explore.html` and `app.js`: Biparjoy (6–19 Jun), Tej (20–24 Oct), Hamoon (21–26 Oct), Midhili (15–18 Nov), Michaung (1–6 Dec).
    - Enforced dates only — no track paths plotted or implied; included explicit note `"(dates only — storm tracks not in dataset)"`.
    - Implemented `initStormReplay()` supporting play/pause cycling and day-by-day scrubbing.
  - [x] **Item 11: Trust Layer (Truth Check Rebranding)**:
    - Rebranded `argo.html` as "Truth Check" (`<title>Kyogre — Truth Check: Model vs Reanalysis vs Argo</title>`), comparing Model vs GLORYS Reanalysis vs in-situ Argo observations with error shown by depth.
    - Added calibrated 90% error bands on depth profile chart: labeled exactly `"typical 90% band, held 89% on 2023"`.
    - Added universal provenance badge on every page (`explore.html`, `argo.html`, `fisheries.html`, `marine-ecology.html`, `fingerprint.html`): `model_v6_satswap_anom_14yr`, test window Jun–Dec 2023, caveats: trained 2010–2020, tested 2023, MLD experimental.
  - [x] **Item 12: Relabeling**:
    - Fisheries mode: labeled explicitly `"physical guide, not a catch prediction"` across PFZ stat card and formula popover.
    - Marine heatwave view: labeled explicitly `"experimental: baseline 2021-23 only"` on MHW stat card and headers.
  - [x] **Item 13: Product Maps on Explore**:
    - Added D20, D26, TCHP, MLD as selectable map layers on `explore.html` with distinct color ramps, legend ticks, and tooltips.
    - Added 2023 climate-state badge across header banners: `"2023 Climate: El Niño, with a positive Indian Ocean Dipole from mid-year"`.
  - [/] **Item 14: Failure Handling & Fallback Demo (Script Ready; Video Recording Pending)**:
    - Land clicks display friendly notice: `"Land coordinate selected: OceanEmbed generates reconstructions exclusively for marine water bodies."`
    - Out-of-window dates display friendly notice: `"Date outside active model window: Currently serving the 14-year model for June–December 2023. Please select a date between 2023-06-01 and 2023-12-31."`
    - Timeouts display friendly notice: `"Request timed out: The inference service took too long to respond. Please check your network and retry."`
    - Authored comprehensive `FALLBACK_DEMO.md` capturing the scene-by-scene script, voiceover timings, and fallback contingency protocols.
    - **Explicit Gap / Pending User Action**: The physical fallback screen-recording (video file) cannot be generated by the coding agent. The user must screen-record each scene using `FALLBACK_DEMO.md` as the shot list.
  - [x] **Item 15: Regression Guard Test**:
    - Added `test_regression_guard.py` scanning `.html`, `.js`, `.tsx`, `.py`, `.md` for banned stale strings (`"0.75"`, `"+20.0"`, `"41 floats"`, `"615"`, `"615 points"`).
    - Passed with zero errors (`PASS: Zero occurrences of banned stale strings found across all customer-facing files`).
  - [ ] **Item 16: Demo Rehearsal (Pending Live User Execution)**:
    - Agent verified all UI paths, buttons, and endpoints referenced in `FALLBACK_DEMO.md` end-to-end (100% pass, zero errors).
    - **Pending User Action**: The live spoken 3x rehearsal with stopwatch timing (the 3rd run being the Ajay-observed dry run) must be performed by the user. Agent cannot perform spoken rehearsals.
  - [x] **Full Verification Matrix (100% Pass)**:
    ...
    - `npm run build`: **PASS (`dist/kyogre-app.js` 92.5kb, 0 errors)**
